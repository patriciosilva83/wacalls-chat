...
func (b *FlowBridge) synthTTSConfig(ctx context.Context, text string, cfg *FlowVoiceConfig) ([]float32, error) {
	...
	case "lovable":
		return b.synthLovableTTS(ctx, text, cfg)
	}
	voice := ""
	if cfg != nil {
...
func (b *FlowBridge) synthConfiguredFallbackTTS(ctx context.Context, text string, cfg *FlowVoiceConfig) ([]float32, error) {
	...
	if strings.TrimSpace(os.Getenv("LOVABLE_API_KEY")) != "" {
		return b.synthLovableTTS(ctx, text, cfg)
	}
...
func (b *FlowBridge) synthLovableTTS(ctx context.Context, text string, cfg *FlowVoiceConfig) ([]float32, error) {
	apiKey := strings.TrimSpace(os.Getenv("LOVABLE_API_KEY"))
	if apiKey == "" {
		return nil, errors.New("LOVABLE_API_KEY não configurada")
	}
	voice := cloudVoice(cfg)
	model := cloudModel(cfg, "openai/gpt-4o-mini-tts")
	return b.synthOpenAICompatibleTTS(ctx, "lovable", "https://ai.gateway.lovable.dev/v1/audio/speech", apiKey, model, voice, text)
}
