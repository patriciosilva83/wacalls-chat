    ...
    <div className="relative flex flex-1 flex-col">
      <header className="flex items-center gap-3 border-b px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          {!isGroup && (
            <CallButtons sessionId={sessionId} chatJid={chatJid} />
          )}
          <Button
            size="sm"
            variant="ghost"
...
// header. The JID local-part already contains the E.164 digits, so we don't
// need a phone-book lookup; the dialer hook handles ringing/UI updates.
const CallButtons = ({ sessionId, chatJid }: { sessionId: string; chatJid: string }) => {
  ...
  const chatDigits = (chatJid.split("@")[0] ?? "").replace(/[^\d]/g, "");
  const activeCall = useCalls((s) =>
    s.calls.find((c) => {
      if (c.sessionId !== sessionId) return false;
      const peerDigits = (c.peer || "").replace(/[^\d]/g, "");
      return peerDigits && chatDigits && peerDigits === chatDigits;
    }),
  );
  // Preserve the original JID server (@lid or @s.whatsapp.net) — many WhatsApp
  // contacts only expose a LID and the backend's parseDialJID handles both
  // forms. Stripping to a "+digits" phone for an @lid contact makes the
  // server attempt an impossible LID lookup ("could not resolve recipient
  // LID — call would not ring").
  const local = (chatJid.split("@")[0] ?? "").split(":")[0] ?? "";
