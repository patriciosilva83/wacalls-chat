...
func (m *CallManager) HandleCallPreAccept(ctx context.Context, node *waBinary.Node, peerJid types.JID) {
	m.mu.Lock()
	call := m.currentCall
	m.mu.Unlock()
	if call == nil {
	...
	m.remotePreaccepted = true
	if call.EncryptionKey != nil {
		m.initSrtpKeysLocked()
	}
	m.mu.Unlock()

	m.log.Info("remote preaccepted call", "call_id", call.CallID, "peer", peerJid.String())
}
...
func (m *CallManager) HandleCallAck(ctx context.Context, node *waBinary.Node) {
	...
	// Do not send <preaccept/> from the call initiator. WhatsApp expects
	// preaccept only from the callee after receiving an offer. Sending it from
	// our side right after the offer ack makes some clients ring and then send an
	// immediate terminate, which appears in the UI as "a chamada foi encerrada
	// antes de iniciar". The outgoing side should only wait for the peer's
	// preaccept/accept and open the relay path from the offer ack.
