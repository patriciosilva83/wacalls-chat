...
REPO_PADRAO="https://github.com/JotaDev66/WaCalls.git"
...
selecionar_zip_backend_go() {
  ...
  while IFS= read -r zip_file; do
    ...
    log_warn "Ignorando zip sem backend Go completo: ${zip_file}"
...
banner() {
  ...
  printf "\n"
  printf "  ${CYAN}Créditos:${WHITE} ${GREEN}JotaDev66${WHITE} • ${GREEN}jobasfernandes${WHITE} • ${GREEN}Canal Vem Fazer${WHITE}\n"
  printf "  ${CYAN}Repositório:${WHITE} ${BLUE}https://github.com/JotaDev66/WaCalls${WHITE}\n"
  printf "  ${CYAN}YouTube:   ${WHITE} ${BLUE}https://www.youtube.com/@vemfazer${WHITE}\n\n"
...
coletar_dados() {
  ...
  if [ -n "${zip_auto}" ] && [ -f "${zip_auto}" ]; then
    ...
    log_err "Nenhum zip encontrado em /root e nenhum código completo em ${APP_DIR}."
    log_err "Coloque o arquivo wacalls.zip em /root e rode o instalador novamente."
    exit 1
  fi
